...
namespace ContainerShip2
{
    public class Balancer
    {
        ...
        public bool IsShipInBalance()
        {
            ...
            {
                for (int y = 0; y < ship.layers[i].length; y++)
                {
                    for (int x = 0; x < ship.layers[i].width; x++)
                    {
                        if (ship.layers[i].layout[x, y] != null)
                        {
                            if (ship.width % 2 == 0) // Even number of columns
                            {
                                if (x < middleLaneStartCol)
                                {
                                    leftWeight += ship.layers[i].layout[x, y].Sum(container => container.Weight);
                                }
                                else
                                {
                                    rightWeight += ship.layers[i].layout[x, y].Sum(container => container.Weight);
                                }
                            }
                            else // Odd number of columns
                            {
                                ...
                                {
                                    middleWeight += ship.layers[i].layout[x, y].Sum(container => container.Weight);
                                }
                            }
                        }
                    }
